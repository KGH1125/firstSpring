...
public class BoardServiceImpl implements BoardService {
	...
	@Override
	public List<BoardUserDTO> gettagList(PagingVO pgvo) {
		List<BoardUserDTO> budtoList = new ArrayList<BoardUserDTO>();
		List<BoardVO> blist = bdao.gettagList(pgvo);
		int i=0;
		for(BoardVO bvo :blist) {
			UserVO uvo = udao.getUser(bvo.getId());
			BoardUserDTO budto = new BoardUserDTO();
	...
	@Override
	public int[] userinfor(String id) {
		int userinfor[]=new int[2];
		userinfor[0]=bdao.selectWrite(id);
		List<BoardVO> list=bdao.selectLike(id);
		int i=0;
		for(BoardVO bvo : list) {
			String username[]=bvo.getLikeUserArr();
			if(username!=null) {
