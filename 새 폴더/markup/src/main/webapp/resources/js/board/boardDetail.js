...
document.getElementById('listBlackBtn').addEventListener('click',()=>{
  if(document.getElementById('listBlackBtn').classList.contains('done')){
    alert('이미 신고가 완료된 게시글입니다.');
  }else{
  confirm('해당 게시글을 신고하시겠습니까?');
  if(confirm){
    document.getElementById('listBlackBtn').classList.add('done');
    console.log('신고 하겠대요');
    let bno = document.getElementById('listBlackBtn').dataset.bno;
    listBlackPlus(bno).then(result=>{
      if(result=='1'){
        alert('신고되었습니다.');
      }
    })
...
document.addEventListener('click',(e)=>{
  if(e.target.classList.contains('blackBtn')){
    if(e.target.classList.contains('done')){
      alert('이미 신고가 완료된 댓글입니다.');
    }else{
      confirm('이 댓글을 신고하시겠습니까?');
      if(confirm){
        let li=e.target.closest('li');
      let cno=li.dataset.cno;
      blackcomment(cno).then(result=>{
        if(result>0){
          alert('신고되었습니다.');
        }
      })
...
async function blackcomment(cno){
  try{
    const url='/comment/blackcomment/'+cno;
    const config ={
      method:'get'
    };
    const resp= await fetch(url, config);
